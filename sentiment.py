...
def load_model():
    ...
    model_file = client.get_object(
        Bucket=os.environ["AWS_S3_BUCKET_NAME"], Key=os.environ["AWS_MODEL_FILE_NAME"]
    )
...
def extract_features(input_string):
    ...
    features = [
        stemmer.stem(word)
        for word in words
        if stemmer.stem(word) not in stopword_list
        and stemmer.stem(word) not in punctuation_list
    ]
...
def analyze(event, context):
    ...
    try:
        ...
        return {
            STATUS_CODE_KEY: 500,
            BODY_KEY: "There was an error processing your request.",
        }
